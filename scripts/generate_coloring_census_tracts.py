#done specifically for Seattle Flu
import argparse
import colorsys
import pandas as pd
...
def mapping(coor, label):
    ...
    location_df = location_df[(location_df['latitude'] >= 47.1619235) & (location_df['latitude'] <= 48.106453)]
    location_df = location_df[(location_df['longitude'] >= -122.676089) & (location_df['longitude'] <= -121.910404)]
...
def color_assignment(lat_df, long_df, output_fname):
    ...
    counter = 0
    color_boundary = lat_df['lat_rank'].max()/36
    for index, row in lat_df.iterrows():
        if round(row['lat_rank']%color_boundary) != 0:
            lat_df.loc[index,'color'] = color_scale[counter]
        elif round(row['lat_rank']%color_boundary) == 0:
            lat_df.loc[index,'color'] = color_scale[counter]
            counter += 1

    #the luminance will vary based on a factor ranging from 0.5 to 1.5 depending on the longitude ranking
    lat_df = lat_df.set_index('location')
    long_df = long_df.set_index('location')
    for index, row in long_df.iterrows():
        factor = (row['long_rank']*(1/long_df['long_rank'].max()))+.25
    ...
    for index, row in lat_df.iterrows():
        ...
        lat_df.loc[index, 'new_color'] = new_hex

    #subsets dataframe to only include location and color and prints to tsv
    final_colors_df = lat_df['new_color']
    final_colors_df.to_csv(output_fname, sep='\t', index = True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default="config/colors_location.tsv", help="name of output file for colors.tsv")
...
color_assignment(lat_df, long_df, args.output)
